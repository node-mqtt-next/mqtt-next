![](media/fcad627b0381c84189f9547fcdedd0d1.jpg)
...
[3.1.3.2.2 Will Delay Interval](#_Toc3901062)
...
[3.2.2.3.15 Response Information](#_Toc3901095)
...
[3.4.2.2.3 User Property](#_Toc3901128)
...
[3.8 SUBSCRIBE - Subscribe request](#_Toc3901161)
...
[3.11.3 UNSUBACK Payload](#_Toc3901194)
...
[3.15.3 AUTH Payload](#_Toc3901227)
...
[4.13.2 Other errors](#_Toc3901260)
...
[Appendix C. Summary of new features in MQTT v5.0 (non-normative)](#_Toc3901293)
...
An error that is detected after the packet has been parsed and found to contain data that is not allowed by the protocol or is inconsistent with the state of the Client or Server. Refer to [section 4.13](#handling-errors) for information about error handling.
...
If the User Name Flag is set to 1, the User Name is the next field in the Payload. The User Name MUST be a UTF-8 Encoded String as defined in [section 1.5.4](#utf-8-encoded-string) [MQTT-3.1.3-12]. It can be used by the Server for authentication and authorization.
...
The Variable Header of the CONNACK Packet contains the following fields in the order: Connect Acknowledge Flags, Connect Reason Code, and Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).
...
If the Server receives a SUBSCRIBE packet containing Shared Subscriptions and it does not support Shared Subscriptions, this is a Protocol Error. The Server uses DISCONNECT with Reason Code 0x9E (Shared Subscriptions not supported) as described in [section 4.13](#handling-errors).
...
Followed by a UTF-8 Encoded String containing the name of the authentication method. It is a Protocol Error to include the Authentication Method more than once. Refer to [section 4.12](#enhanced-authentication) for more information about extended authentication.
...
The Variable Header of the PUBLISH Packet contains the following fields in the order: Topic Name, Packet Identifier, and Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).

#### Topic Name

The Topic Name identifies the information channel to which Payload data is published.

The Topic Name MUST be present as the first field in the PUBLISH packet Variable Header. It MUST be a UTF-8 Encoded String as defined in [section 1.5.4](#utf-8-encoded-string) [MQTT-3.3.2-1].

The Topic Name in the PUBLISH packet MUST NOT contain wildcard characters [MQTT-3.3.2-2].

The Topic Name in a PUBLISH packet sent by a Server to a subscribing Client MUST match the Subscription’s Topic Filter according to the matching process defined in [section 4.7](#topic-names-and-topic-filters) [MQTT-3.3.2-3]. However, as the Server is permitted to map the Topic Name to another name, it might not be the same as the Topic Name in the original PUBLISH packet.

To reduce the size of the PUBLISH packet the sender can use a Topic Alias. The Topic Alias is described in [section 3.3.2.3.4](#topic-alias). It is a Protocol Error if the Topic Name is zero length and there is no Topic Alias.

#### Packet Identifier

The Packet Identifier field is only present in PUBLISH packets where the QoS level is 1 or 2. [Section 2.2.1](#packet-identifier) provides more information about Packet Identifiers.
...
A Server MUST send the Payload Format Indicator unaltered to all subscribers receiving the Application Message [MQTT-3.3.2-4]. The receiver MAY validate that the Payload is of the format indicated, and if it is not send a PUBACK, PUBREC, or DISCONNECT with Reason Code of 0x99 (Payload format invalid) as described in [section 4.13](#handling-errors). Refer to [section 5.4.9](#handling-of-disallowed-unicode-code-points) for information about security issues in validating the payload format.<#Unicode><#RFC3629>
...
The PUBLISH packet sent to a Client by the Server MUST contain a Message Expiry Interval set to the received value minus the time that the Application Message has been waiting in the Server [MQTT-3.3.2-6]. Refer to [section 4.1](#session-state) for details and limitations of stored state.
...
Followed by a UTF-8 Encoded String describing the content of the Application Message. The Content Type MUST be a UTF-8 Encoded String as defined in [section 1.5.4](#utf-8-encoded-string) [MQTT-3.3.2-19].
...
Figure 39 - PUBLISH packet Variable Header non-normative example
...
Table 33 Expected PUBLISH packet response
...
If a Client receives an unsolicited Application Message (not resulting from a subscription) which has a QoS greater than Maximum QoS, it uses a DISCONNECT packet with Reason Code 0x9B (QoS not supported ) as described in [section 4.13](#handling-errors) Handling errors.
...
1.  A Topic Alias value of 0 or greater than the Maximum Topic Alias is a Protocol Error, the receiver uses DISCONNECT with Reason Code of 0x94 (Topic Alias invalid) as described in [section 4.13.](#handling-errors)
...
3.  If the receiver does not already have a mapping for this Topic Alias
    1.  If the packet has a zero length Topic Name field it is a Protocol Error and the receiver uses DISCONNECT with Reason Code of 0x82 (Protocol Error) as described in [section 4.13](#handling-errors).
...
Figure 310 - PUBACK packet Fixed Header
...
The Variable Header of the PUBACK Packet contains the following fields in the order: Packet Identifier from the PUBLISH packet that is being acknowledged, PUBACK Reason Code, Property Length, and the Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).

Figure 311 – PUBACK packet Variable Header
...
Figure 312 - PUBREC packet Fixed Header
...
The Variable Header of the PUBREC Packet consists of the following fields in the order: the Packet Identifier from the PUBLISH packet that is being acknowledged, PUBREC Reason Code, and Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).

Figure 313 - PUBREC packet Variable Header
...
Figure 314 – PUBREL packet Fixed Header
...
The Variable Header of the PUBREL Packet contains the following fields in the order: the Packet Identifier from the PUBREC packet that is being acknowledged, PUBREL Reason Code, and Properties. The rules for encoding Properties are described in [section 2.2.2.](#properties)

Figure 315 – PUBREL packet Variable Header
...
Figure 316 – PUBCOMP packet Fixed Header
...
The Variable Header of the PUBCOMP Packet contains the following fields in the order: Packet Identifier from the PUBREL packet that is being acknowledged, PUBCOMP Reason Code, and Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).

Figure 317 - PUBCOMP packet Variable Header
...
Figure 318 SUBSCRIBE packet Fixed Header
...
The Variable Header of the SUBSCRIBE Packet contains the following fields in the order: Packet Identifier, and Properties. [Section 2.2.1](#packet-identifier) provides more information about Packet Identifiers. The rules for encoding Properties are described in [section 2.2.2](#properties).

**Non-normative example**

Figure 3-19 shows an example of a SUBSCRIBE variable header with a Packet Identifier of 10 and no properties.

Figure 319 – SUBSCRIBE Variable Header example
...
Refer to [section 3.8.3.1](#subscription-options) for more information about the handling of Subscription Identifiers.
...
The Payload MUST contain at least one Topic Filter and Subscription Options pair [MQTT-3.8.3-2]. A SUBSCRIBE packet with no Payload is a Protocol Error. Refer to [section 4.13](#handling-errors) for information about handling errors.
...
Figure 320– SUBSCRIBE packet Payload format
...
Figure 321 - Payload byte format non-normative example
...
If a Server receives a Nonshared Topic Filter that is not identical to any Topic Filter for the current Session, a new Non-shared Subscription is created. If the Retain Handling option is not 2, all matching retained messages are sent to the Client.
...
Refer to [section 4.8](#shared-subscriptions) for more details on Shared Subscriptions.
...
Figure 322 - SUBACK Packet Fixed Header
...
Figure 323 SUBACK packet Variable Header
...
Table 38 - Subscribe Reason Codes
...
The Variable Header of the UNSUBSCRIBE Packet contains the following fields in the order: Packet Identifier, and Properties. [Section 2.2.1](#packet-identifier) provides more information about Packet Identifiers. The rules for encoding Properties are described in [section 2.2.2](#properties).
...
The Payload for the UNSUBSCRIBE packet contains the list of Topic Filters that the Client wishes to unsubscribe from. The Topic Filters in an UNSUBSCRIBE packet MUST be UTF-8 Encoded Strings [MQTT-3.10.3-1] as defined in [section 1.5.4](#utf-8-encoded-string), packed contiguously.

The Payload of an UNSUBSCRIBE packet MUST contain at least one Topic Filter [MQTT-3.10.3-2]. An UNSUBSCRIBE packet with no Payload is a Protocol Error. Refer to [section 4.13](#handling-errors) for information about handling errors.
...
The Variable Header of the UNSUBACK Packet the following fields in the order: the Packet Identifier from the UNSUBSCRIBE Packet that is being acknowledged, and Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).
...
Table 39 - Unsubscribe Reason Codes
...
This packet is used in Keep Alive processing. Refer to [section 3.1.2.10](#keep-alive) for more details.
...
The Client or Server MUST validate that reserved bits are set to 0. If they are not zero it sends a DISCONNECT packet with a Reason code of 0x81 (Malformed Packet) as described in [section 4.13](#handling-errors) [MQTT-3.14.1-1].
...
The Variable Header of the DISCONNECT Packet contains the following fields in the order: Disconnect Reason Code, and Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).
...
Table 310 – Disconnect Reason Code values
...
The Server sends DISCONNECT including a Server Reference and Reason Code 0x9C (Use another server) or 0x9D (Server moved) as described in [section 4.13](#handling-errors).

Refer to [section 4.11](#server-redirection) Server Redirection for information about how Server Reference is used.

Figure 324 DISCONNECT packet Variable Header non-normative example
...
-   MUST discard any Will Message associated with the current Connection without publishing it [MQTT-3.14.4-3], as described in [section 3.1.2.5](#will-flag).
...
The Variable Header of the AUTH Packet contains the following fields in the order: Authenticate Reason Code, and Properties. The rules for encoding Properties are described in [section 2.2.2](#properties).
...
Table 311 Authenticate Reason Codes
...
Followed by a UTF-8 Encoded String containing the name of the authentication method. It is a Protocol Error to omit the Authentication Method or to include it more than once. Refer to [section 4.12](#enhanced-authentication) for more information about extended authentication.

##### Authentication Data

**22 (0x16) Byte**, Identifier of the Authentication Data.

Followed by Binary Data containing authentication data. It is a Protocol Error to include Authentication Data more than once. The contents of this data are defined by the authentication method. Refer to [section 4.12](#enhanced-authentication) for more information about extended authentication.
...
This Quality of Service level ensures that the message arrives at the receiver at least once. A QoS 1 PUBLISH packet has a Packet Identifier in its Variable Header and is acknowledged by a PUBACK packet. [Section 2.2.1](#packet-identifier) provides more information about Packet Identifiers.
...
A QoS 2 message has a Packet Identifier in its Variable Header. [Section 2.2.1](#packet-identifier) provides more information about Packet Identifiers. The receiver of a QoS 2 PUBLISH packet acknowledges receipt with a two-step acknowledgement process.
...
-   MUST treat the PUBLISH packet as “unacknowledged” until it has received the corresponding PUBREC packet from the receiver [MQTT-4.3.3-3]. Refer to [section 4.4](#message-delivery-retry) for a discussion of unacknowledged messages.
...
Note that a sender is permitted to send further PUBLISH packets with different Packet Identifiers while it is waiting to receive acknowledgements, subject to flow control as described in [section 4.9](#flow-control).
...
When a Server takes ownership of an incoming Application Message it MUST add it to the Session State for those Clients that have matching Subscriptions [MQTT-4.5.0-1]. Matching rules are defined in [section 4.7.](#topic-names-and-topic-filters)
...
The following these rules apply to the Client when implementing the protocol flows defined in [section 4.3.](#quality-of-service-levels-and-protocol-flows)
...
By default, a Server MUST treat every Topic as an Ordered Topic when it is forwarding messages on Nonshared Subscriptions. [MQTT-4.6.0-6]. A Server MAY provide an administrative or other mechanism to allow one or more Topics to not be treated as an Ordered Topic.
...
If both Client and Server set Receive Maximum to 1, they make sure that no more than one message is “in-flight” at any one time. In this case no QoS 1 message will be received after any later one even on re-connection. For example a subscriber might receive them in the order 1,2,3,3,4 but not 1,2,3,2,3,4. Refer to [section 4.9](#flow-control) Flow Control for details of how the Receive Maximum is used.
...
-   Topic Names and Topic Filters are UTF-8 Encoded Strings; they MUST NOT encode to more than 65,535 bytes [MQTT-4.7.3-3]. Refer to [section 1.5.4](#utf-8-encoded-string).
...
-   {filter} The remainder of the string has the same syntax and semantics as a Topic Filter in a non-shared subscription. Refer to [section 4.7](#topic-names-and-topic-filters).
-   

A Shared Subscription's Topic Filter MUST start with \$share/ and MUST contain a ShareName that is at least one character long [MQTT-4.8.2-1]. The ShareName MUST NOT contain the characters "/", "+" or "\#", but MUST be followed by a "/" character. This "/" character MUST be followed by a Topic Filter [MQTT-4.8.2-2] as described in [section 4.7](#topic-names-and-topic-filters).
...
-   If the Server is in the process of sending a QoS 2 message to its chosen subscribing Client and the connection to the Client breaks before delivery is complete, the Server MUST complete the delivery of the message to that Client when it reconnects [MQTT-4.8.2-4] as described in [section 4.3.3](#qos-2-exactly-once-delivery). If the Client's Session terminates before the Client reconnects, the Server MUST NOT send the Application Message to any other subscribed Client [MQTT-4.8.2-5].
...
Clients and Servers control the number of unacknowledged PUBLISH packets they receive by using a Receive Maximum value as described in [section 3.1.2.11.4](#receive-maximum) and [section 3.2.2.3.2](#receive-maximum-1). The Receive Maximum establishes a send quota which is used to limit the number of PUBLISH QOS \> 0 packets which can be sent without receiving an PUBACK (for QoS 1) or PUBCOMP (for QoS 2). The PUBACK and PUBCOMP replenish the quota in the manner described below.
...
Refer to [section 3.3.4](#publish-actions) for a description of how Clients and Servers react if they are sent more PUBLISH packets than the Receive Maximum allows.
...
-   Correlation Data, described in [section 3.3.2.3.6](#correlation-data-1)
-   Request Response Information, described in [section 3.1.2.11.7](#request-response-information)
-   Response Information, described in [section 3.2.2.3.14](#response-information)

The following non-normative sections describe how these properties can be used.

A Client sends a Request Message by publishing an Application Message which has a Response Topic set as described in [section 3.3.2.3.5](#response-topic-1). The Request can include a Correlation Data property as described in [section 3.3.2.3.6](#correlation-data-1).
...
Refer to [section 3.1.2.11.7](#response-information) for the definition of the Response Information.

## Server redirection

A Server can request that the Client uses another Server by sending CONNACK or DISCONNECT with Reason Codes 0x9C (Use another server), or 0x9D (Server moved) as described in [section 4.13](#handling-errors). When sending one of these Reason Codes, the Server MAY also include a Server Reference property to indicate the location of the Server or Servers the Client SHOULD use.
...
To begin an enhanced authentication, the Client includes an Authentication Method in the CONNECT packet. This specifies the authentication method to use. If the Server does not support the Authentication Method supplied by the Client, it MAY send a CONNACK with a Reason Code of 0x8C (Bad authentication method) or 0x87 (Not Authorized) as described in [section 4.13](#handling-errors) and MUST close the Network Connection [MQTT-4.12.0-1].
...
The Client can close the connection at any point in this process. It MAY send a DISCONNECT packet before doing so. The Server can reject the authentication at any point in this process. It MAY send a CONNACK with a Reason Code of 0x80 or above as described in [section 4.13](#handling-errors), and MUST close the Network Connection [MQTT-4.12.0-4].
...
If the re-authentication fails, the Client or Server SHOULD send DISCONNECT with an appropriate Reason Code as described in [section 4.13](#handling-errors), and MUST close the Network Connection [MQTT-4.12.1-2].
...
Definitions of Malformed Packet and Protocol Errors are contained in [section 1.2](#terminology) Terminology, some but not all, of these error cases are noted throughout the specification. The rigor with which a Client or Server checks an MQTT Control Packet it has received will be a compromise between:
...
If the sender is compliant with this specification it will not send Malformed Packets or cause Protocol Errors. However, if a Client sends MQTT Control Packets before it receives CONNACK, it might cause a Protocol Error because it made an incorrect assumption about the Server capabilities. Refer [to section 3.1.4](#connect-actions) CONNECT Actions.
...
MQTT v5.0 provides an enhanced authentication mechanism as described in [section 4.12](#enhanced-authentication). Using this requires support for it in both the Client and Server.<#RFC4511><#RFC6749>
...
An implementation might allow for authentication where the credentials are sent in an Application Message from the Server to the Client. MQTT v5.0 provides an enhanced authentication mechanism as described in [section 4.12](#enhanced-authentication)., which can be used to Authenticate the Server to the Client. Using this requires support for it in both the Client and Server.
...
Client and Server implementations using TLS can choose to provide capabilities to check Certificate Revocation Lists (CRLs [[RFC5280]](#RFC5280)) and Online Certificate Status Protocol (OSCP**)** [[RFC6960]](#RFC6960) **t**o prevent revoked certificates from being used.<#RFC5246><#RFC5280><#RFC6960>
...
-   Servers may require their Client to reauthenticate periodically using the mechanism described in [section 4.12.1](#re-authentication).
...
Figure 6.61 - IANA WebSocket Identifier
...
1.  The format of all MQTT Control Packets that the Server sends matches the format described in [Chapter 2](#mqtt-control-packet-format) and [Chapter 3](#mqtt-control-packets).
2.  It follows the Topic matching rules described in [section 4.7](#topic-names-and-topic-filters) and the Subscription rules in [section 4.8](#subscriptions).
...
-   [Chapter 6 - Using WebSocket as a network transport](#using-websocket-as-a-network-transport)
1.  It does not require the use of any extensions defined outside of the specification in order to interoperate with any other conformant implementation.
...
1.  The format of all MQTT Control Packets that the Client sends matches the format described in [Chapter 2](#mqtt-control-packet-format) and [Chapter 3](#mqtt-control-packets).
...
-   [Chapter 3 - MQTT Control Packets](#mqtt-control-packets)
-   [Chapter 4 - Operational behavior](#operational-behavior)
-   [Chapter 6 - Using WebSocket as a network](#using-websocket-as-a-network-transport) transport
1.  It does not require the use of any extensions defined outside of the specification in order to interoperate with any other conformant implementation.
1.  Acknowledgments
...
1.  Mandatory normative statement (non-normative)

This Appendix is non-normative and is provided as a convenient summary of the numbered conformance statements found in the main body of this document. Refer to [Chapter 7](#conformance) for a definitive list of conformance requirements.
...
| [MQTT-3.8.4-3]                 | If a Server receives a SUBSCRIBE packet containing a Topic Filter that is identical to a Nonshared Subscription’s Topic Filter for the current Session then it MUST replace that existing Subscription with a new Subscription.                                                                                                                                                                     |
...
1.  Summary of new features in MQTT v5.0 (non-normative)
